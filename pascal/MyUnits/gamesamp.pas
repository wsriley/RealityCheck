...
var x,y:integer;
 ...
 begin
      directvideo:=false;
      gotoxy(1,1);textattr:=random(255);writeln('DEMO USE ONLY');
...
Begin
  ...
  setgraph;
  textattr:=lightcyan;
  gotoxy(7,3);writeln('* * * FSPRITE v2.0 * * * ');
  textattr:=cyan;
  gotoxy(11,5);writeln('COPYRIGHT 1993');
  textattr:=white;
  gotoxy(2,7);writeln('WHITE OBSDIAN SOFTWARE & STEVE RILEY');
  textattr:=lightgray;
  gotoxy(9,10);writeln('EVALUATION COPY ONLY.');
  textattr:=lightblue;
  gotoxy(1,12);writeln('This Version Not for Professional Use.');
  gotoxy(3,14);writeln('For Registration See Documentation.');
  delay(4000);
  New(Mid); New(Back);
